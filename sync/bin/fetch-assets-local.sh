...
download_root="${DOWNLOAD_ROOT:-$script_dir/../data/local-releases}"
...
while IFS= read -r release_json; do
  ...
  while IFS=$'\t' read -r asset_name asset_url asset_size; do
    ...
    if [ -f "$release_dir/$asset_name" ] && [ "$force_download" != "1" ]; then
      existing_size="$(file_size_bytes "$release_dir/$asset_name")"

      if [ "$existing_size" = "$asset_size" ]; then
        log_info "skip existing asset: $tag_name/$asset_name"
      else
        log_info "redownload asset with size mismatch: $tag_name/$asset_name"
...
log_info "local fetch assets finished"